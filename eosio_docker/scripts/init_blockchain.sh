...
# Active key for account4sale
cleos wallet import -n notechainwal --private-key 5KLqT1UFxVnKRWkjvhFur4sECrPhciuUqsYRihc1p9rxhXQMZBg
# Active key for sarah
cleos wallet import -n notechainwal --private-key 5K2jun7wohStgiCDSDYjk3eteRH1KaxUQsZTEmTGPH4GS9vVFb7
...
cleos system newaccount eosio --transfer tradeaccount EOS8BCgapgYA2L4LJfCzekzeSr3rzgSTUXRXwNi8bNRoz31D14en9 \
--stake-net "1.0000 EOS" --stake-cpu "1.0000 EOS" --buy-ram-kbytes 138675


echo "=== give eosio.code permission to tradeaccount ==="
cleos set account permission tradeaccount active \
'{"threshold": 1,"keys": [{"key": "EOS8BCgapgYA2L4LJfCzekzeSr3rzgSTUXRXwNi8bNRoz31D14en9","weight": 1}],"accounts": [{"permission":{"actor":"tradeaccount","permission":"eosio.code"},"weight":1}]}' \
owner -p tradeaccount

# Moved into 'run_tradeaccount.sh'
...
cleos push action eosio.token transfer '[ "eosio", "sarah", "10000.0000 EOS", "memo"]' -p eosio
...
deploy_contract.sh tradeaccount tradeaccount notechainwal $(cat notechain_wallet_password.txt)
